...
rem Clear the CLASSPATH variable to start with
set CLASSPATH=
...
set CLASSPATH=%CLASSPATH%;%ENDECA_ROOT%\lib\java\orawsdl.jar
set CLASSPATH=%CLASSPATH%;%ENDECA_ROOT%\lib\java\wsdl4j-to-orawsdl-1.0.0.jar
...
rem Add CAS jars to the classpath
if defined CAS_ROOT (
  set CLASSPATH=%CLASSPATH%;%CAS_ROOT%\lib\cas-dt\casStubs.jar
) else (
  echo WARNING: Set CAS_ROOT correctly. Ignore this warning, if not using CAS based indexing.
)
...
for %%i in ("%~dp0\..\config\lib\java\*.zip") do call set CLASSPATH=%%CLASSPATH%%;%%i%%
