...
echo Importing content using public format...
call %~dp0runcommand.bat IFCR importContent pages %~dp0..\config\import\pages
if not %ERRORLEVEL%==0 (
    echo Failed to import content in public format.
    exit /b 1
)

echo Importing content using legacy format...
...
echo Finished importing content in legacy format
