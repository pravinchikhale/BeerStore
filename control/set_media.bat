...
rem Replace all media with files in \config\ifcr\media folder.
for /F %%i in ('dir /b "%~dp0..\config\ifcr\media\*.*"') do (
  call %~dp0runcommand.bat IFCR importNode %~dp0..\config\ifcr\media media
  if not %ERRORLEVEL%==0 (
    exit /b 1
  )
  goto :EOF
