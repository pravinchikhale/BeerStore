...
rem  Replace all editors config with files in ..\config\import\configuration\tools\xmgr folder.
call %~dp0runcommand.bat IFCR importContent editors %~dp0..\config\import\editors
if not %ERRORLEVEL%==0 (
  exit /b 1
)

rem Replace configuration from ..\config\import\configuration\tools\xmgr
call %~dp0runcommand.bat IFCR importContent configuration/tools/xmgr %~dp0..\config\import\configuration\tools\xmgr
if not %ERRORLEVEL%==0 (
  exit /b 1
)